...
const Projects = () => {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center  gap-10  px-8 pt-32  max-[]: pb-15 font-mono relative">

      <div className={"absolute top-18  text-2xl  font-bold tracking-wide text-black/85 " + zoom(animate)}>
        Projects
      </div>
      ...
      <div className={"w-full max-w-6xl flex flex-col lg:flex-row gap-6" + zoom(animate)}>
        <div className="max-[1000px]:h-auto w-full lg:w-1/3  rounded-xl bg-[#EFE9E3]/40 shadow-lg  p-4 flex flex-col  gap-5  pt-17
         relative ">
          <div className='absolute top-2 left-0 flex flex-col justify-center gap-1 p-4 font-bold  text-xl h-12 w-full text-black/85'>
            <div>
          ...
          {Object.values(details).map((project, i) => (
            <div
              ...
              className={`h-auto w-auto   ${topDown(animateProjects)} `}
              style={{
                transitionDelay: animateProjects ? `${i * 200}ms` : '0ms'
              }}
            >
              <div className={`rounded-lg px-3 py-2 cursor-pointer transition-all duration-500  ${projectIndex === i
                ? ' bg-[#C9B59C] shadow-md scale-[1.02]'
                : ' bg-[#F9F8F6] hover:bg-[#d9cabb] opacity-80 hover:opacity-100'
                } `}>
                <div className="text-xl max-[500px]:text-sm font-semibold text-black/85 mb-2">
                  {project.name}
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {project.tech.map((element, indexStack) => (
                    <div
                      key={indexStack}
                      className={"px-2  py-0.5 rounded-sm max-[500px]:text-[10px] text-[12px] bg-black/85 text-white hover:bg-black transition-colors duration-300" + ((indexStack > 2) ? " hidden" : "")}
                    >
                      {element}
                    </div>
                  ))}
                  {
                    <div
                      className={"px-2 py-0.5 rounded-sm max-[500px]:text-[10px] text-[12px] bg-black/85 text-white hover:bg-black transition-colors duration-300" + ((project.tech.length < 3) ? " hidden" : "")}
                    >
                      {
        ...
        <div className=" h-[500px] max-[770px]: w-full lg:w-2/3 rounded-xl bg-[#EFE9E3]/40 shadow-lg flex flex-col items-center justify-start gap-1 p-4 relative pt-20 pb-14 max-[1000px]:pb-18 max-[1000px]:h-auto">
          <div className='absolute top-2 left-0 flex flex-col justify-center items-center gap-1 font-bold  text-xl h-12 w-full text-black/85'>
            <div>
              Description
            </div>
            <div className='w-full border-1'></div>
          ...
          <div className="w-full h-auto text-black/95 text-sm shrink-0 min-h-25 mb-4" dangerouslySetInnerHTML={{ __html: currentProject.discription }}>
          </div>
          <div className='flex items-center pt-2 gap-1 text-sm w-full flex-wrap'>
            {currentProject.tech.map((element, indexStack) => (
              <div
                key={indexStack}
                className="px-2 py-0.5 w-auto rounded-sm text-[12px] max-[500px]:text-[10px] cursor-pointer bg-black/85 text-white tracking-wide hover:bg-black transition-colors duration-300 flex items-center justify-center shrink-0"
              >
                {element}
              </div>
            ))}
